...
|一意|:x:|
## デバイス(device)
クラスのキー属性はdeviceid です。
このクラスの属性は以下のとおりです。
### ユーザ名(uid)
ユーザ名(uid) はシステムの利用者です。
| データ型|string|
|----|----------------|
|必須|:o:|
|一意|:x:|
### MACアドレス(macaddr)
MACアドレス(macaddr) はデバイスのMACアドレスです。
| データ型|string|
|----|----------------|
|必須|:o:|
|一意|:x:|
...
| 最小長|12|
### 資産管理番号(deviceid)
資産管理番号(deviceid) はデバイスを管理する番号です。
| データ型|string|
|----|----------------|
|必須|:o:|
|一意|:o:|
### 登録日(registrationDate)
登録日(registrationDate) はデバイスを登録した日付です。
| データ型|date|
|----|----------------|
|必須|:x:|
|一意|:x:|
### 状態(status)
状態(status) はクライアント証明書発行状態です。
| データ型|string|
|----|----------------|
|必須|:x:|
|一意|:x:|
### 通知先メールアドレス(notificationEmailAddr)
| データ型|stringの配列|
|----|----------------|
|必須|:x:|
|一意|:x:|
