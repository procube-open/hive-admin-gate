...
get_connections_path = '/api/session/data/' + guacamole_database + '/connections?token=' + auth_token
...
for v in connections_data.values():
  if v["idmIdentifier"] == "session-manager" and v["protocol"] == "vnc" and v["activeConnections"] == 0:
    print("delete " + v["name"])
    delete_url = 'http://localhost:80/delete'
    params = {
      "work_container": v["name"],
      "vnc_identifier": v["identifier"]
    }
